...
def process_data(datafile):
    ...
    for p in planes:
        ...
        np.savetxt(datafile.split("/")[-1].split(".")[0].split("_")[0]+"_"+str(p[0])+str(p[1])+str(p[2])+".csv",np.vstack((kvs,sf)).T,delimiter=',')
...
with parallel_config(backend='loky', n_jobs=len(sims)):
   Parallel()(delayed(process_data)(df) for df in sims)
