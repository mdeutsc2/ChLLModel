	  ...
	  integer qq(mi,mj,mk)
	  integer dope(mi,mj,mk)
	  integer kount(mk)
	  integer nconfigs
	  real*8 nx(mi,mj,mk),ny(mi,mj,mk)
	  real*8 nz(mi,mj,mk)
	  real*8 nxnew(mi,mj,mk),nynew(mi,mj,mk),nznew(mi,mj,mk)
	  real*8 crossx,crossy,crossz,dott
	  real*8 nnx,nny,nnz
	  real*8 d,KK,phitop,rho
	  real*8 rhoz(mk)
	  open(unit=10,file='LLMC-surf01a.dat',status='unknown')
	  open(unit=16,file='LLMC-s01a-rhoz-T-0.05.dat',status='unknown')
