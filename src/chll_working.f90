program chll
  ...
  integer(int64) :: i,j,k,istep,itry
    integer(int64) :: ip1,im1,jp1,jm1,kp1,km1,snew
    real(real64) :: nnx,nny,nnz,eold,enew,ux,uy,uz,rsq,vx,vy,vz,dcosphi,dsinphi,xxnew,yynew,zznew,rnd
    ...
    if (enew.lt.eold) then
     ...
     do concurrent(itry = 1:nsub)
        ...
        if (dabs(nnz).gt.0.999d0) then
           phi = rand1(itry,1)*8.0d0*datan(1.0d0)
           xxnew = nnx+d*dcos(phi)
           yynew = nny+d*dsin(phi)
           zznew = nnz
           rsq = dsqrt(xxnew*xxnew+yynew*yynew+zznew*zznew)
...
contains
	...
	subroutine evolve(sl,nx,ny,nz,s,KK,d)
