...
K = 0.75
for temp in np.arange(0.05,0.501,0.01):
        simename = "smallkbt_K"+str(round(K,3))+"_kbt"+str(round(temp,5))
        sim = chll.ChLLSim(name = simename,ni=128,nj=128,nk=128,kbt=temp,d = 0.01,KK = K,g = 0.0,rho = 0.0)
        init_aligned(sim)
        sim.init()
